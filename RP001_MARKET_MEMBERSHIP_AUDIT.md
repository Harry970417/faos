...
- **Eligibility-gate logic:** applied uniformly to this batch by construction (same code path, no market-conditional branching exists in `RP001_DAILY_INVESTABLE_UNIVERSE_SPEC_v2.md`'s implementation) — no violation surfaced.
- **Missingness:** TPEx stocks in this batch show materially higher missing-rate-warning incidence (95/120, 79%, vs. 20-49% in the all-or-mostly-TWSE Batches 1-9) — plausibly TPEx's generally thinner institutional trading coverage, not a schema or eligibility defect; governed by `RP001_MISSINGNESS_POLICY.md` as designed.
