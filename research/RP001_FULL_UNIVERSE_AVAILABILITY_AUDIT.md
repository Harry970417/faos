...
- **Reconciliation against the pre-registered period** (`research/RP001_RESEARCH_DESIGN.md` §2: "建議期間 2015–2026，實際起點依資料可得性調整"): the 2012-05-02 institutional-data floor is **before** the suggested 2015 start, so **no deviation is required on this point** — the pre-registered period is fully executable as originally suggested.
...
**Verified via the real pilot pull, not a rate-limit stress test.** The 7-stock, 14-request pilot (`research/rp001_pilot_pull.py`, moderate pace, 0.4s inter-request delay) completed with **0 retries, 0 failures, 14/14 HTTP 200** across both `TaiwanStockInstitutionalInvestorsBuySell` and `TaiwanStockPrice`. No rate-limiting behavior (HTTP 429, throttling, or degraded response) was observed at this scale. A dedicated high-throughput stress test (the pace required for a ~2,000+ stock full pull) was **not** run in this audit — the pilot's pace-per-request is the basis for the Capacity Estimate's time projection, with retry-cost margin built in per `research/RP001_PHASE2A_CAPACITY_ESTIMATE.md`, not a verified upper bound on FinMind's actual rate limit.
...
1. The confirmatory test window is anchored to the pre-registered break interval (late-Aug to late-Oct 2025, point estimate 2025-09-25, per `research/RP001_FULL_UNIVERSE_SPEC.md`) — over a decade after the 2014-12-01 cutover.
...
> **Correction note (added 2026-07-31, Phase 2A.2-R, `research/RP001_INSTITUTIONAL_SCHEMA_AUDIT.md` §9):** Point 2 above is factually wrong and is preserved here unedited, not deleted. `FEATURE_REGISTRY.md` line 11 confirms F_INST_01 is defined solely on `Foreign_Investor`; `Dealer_self`/`Dealer_Hedging` are the defining inputs of F_INST_03/F_INST_04, two separate, non-Frozen features not used by any of H-C1–H-C5. This error is what caused Deviation D-04 to escalate on the (incorrect) grounds that Dealer-category drift threatens F_INST_01's constructibility. D-04's Deviation Log entry carries a matching correction note. F_INST_01 was never at risk from this finding.
