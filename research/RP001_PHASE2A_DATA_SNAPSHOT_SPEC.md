...
**Status: Specification only. No full-universe bulk download begins until the Readiness Gate (`research/RP001_PHASE2A_READINESS_GATE.md`) passes.** This document defines the directory layout and manifest schema for Phase 2A.2's data pull; it does not execute it. The pilot (`research/RP001_PHASE2A_DATA_QUALITY_PILOT.md`) already exercises this pattern at small scale under `rp001_data/pilot/` — this spec formalizes and extends it to full-universe scope.
...
This is a direct extension of the pattern already used and proven in the pilot (`pilot_pull_log.csv` — same fields: query params, row count, elapsed time, retries, SHA-256, file size), scaled from 14 rows to the full-universe request count estimated in `research/RP001_PHASE2A_CAPACITY_ESTIMATE.md`.
...
2. `pre_listing_data_flags.csv` — per-stock flag where "first date with data" differs materially from "official listing_date" (the 6986-type hazard documented in `research/RP001_DAILY_UNIVERSE_VALIDATION.md`), so universe construction can be checked against this list rather than trusting the join silently.
...
Creating these directories and beginning the bulk pull is Phase 2A.2, step 4 in `research/RP001_PHASE2A_EXECUTION_PLAN.md` — gated on Protocol approval (already given) and this Readiness Gate (`research/RP001_PHASE2A_READINESS_GATE.md`) passing. This document is the blueprint, not the trigger.
