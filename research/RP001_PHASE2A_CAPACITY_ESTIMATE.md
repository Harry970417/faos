...
**Based on real pilot measurements, not theoretical throughput.** Source: `rp001_data/pilot/pilot_pull_log.csv` (7 stocks × 2 datasets, `research/RP001_PHASE2A_DATA_QUALITY_PILOT.md`) plus one additional real single-stock measurement of the third required dataset (`TaiwanStockPER`, stock 1101) taken during this estimate's preparation.
...
**Combined per-stock, 3 datasets** (institutional + price + valuation — all three are needed: valuation for the locked PBR value/growth definition per `research/RP001_FULL_UNIVERSE_SPEC.md`):
- Rows: ~15,284/stock
...
| Mid | 2,300 | + delisted stocks not already in current registries (337 total delisted per `research/RP001_FULL_UNIVERSE_SPEC.md`, conservatively ~320 net-new) |
