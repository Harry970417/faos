...
| Delisted stock | Covered by existing Deviation D-02 (price-first-observation proxy, since registries exclude delisted names entirely) | `research/RP001_PHASE2A_DEVIATION_LOG.md` D-02 |
| Re-listed stock | **Not observed in the current sample** | — |
| Renamed, code unchanged | Not separately tested — `stock_id` is the study's persistent join key throughout (per Phase 2A.1's own Item 8 note), so a rename alone does not affect eligibility-date logic | `research/RP001_FULL_UNIVERSE_AVAILABILITY_AUDIT.md` Item 8 |
...
## 6. Correction note (2026-08-02, Phase 2A.2 Batch 10): "Ordinary TPEx stock" now verified

§3's "not verified — no TPEx stock present" finding is **superseded, not deleted** (original row preserved above for the record). Batch 10 of Phase 2A.2 acquisition (`rp001_data/phase2a_acquisition_universe.csv` idx 1080-1200) is 111/120 TPEx stocks — the first batch with substantial TPEx representation. Findings:
- **Schema:** identical institutional-category set to TWSE (`Foreign_Investor`, `Dealer`, `Dealer_self`, `Dealer_Hedging`, `Investment_Trust`, `Foreign_Dealer_Self`) — the batch's Integrity Gate raised zero schema-drift flags across all 120 stocks. F_INST_01's constructibility is confirmed market-agnostic, not just a TWSE property.
- **Eligibility-gate logic:** applied uniformly to this batch by construction (same code path, no market-conditional branching exists in `research/RP001_DAILY_INVESTABLE_UNIVERSE_SPEC_v2.md`'s implementation) — no violation surfaced.
- **Missingness:** TPEx stocks in this batch show materially higher missing-rate-warning incidence (95/120, 79%, vs. 20-49% in the all-or-mostly-TWSE Batches 1-9) — plausibly TPEx's generally thinner institutional trading coverage, not a schema or eligibility defect; governed by `research/RP001_MISSINGNESS_POLICY.md` as designed.

"TWSE→TPEx transfer" and "Re-listed stock" remain unverified — no case of either has appeared in any batch through Batch 10. Still an open item, not assumed resolved by extension from the TPEx finding above.
