...
**Alpha 0.1 is the snapshot at the moment Design Approval was granted** — it bundles Knowledge Base **v0.2** (the Baseline), not the remediated output produced immediately after. Knowledge Graph Remediation v0.1 (`architecture/REMEDIATION_REPORT_v0.1.md`, `knowledge/knowledge_base_remediated_v0.1.psv`) was executed under Alpha 0.1's approval but is not retroactively folded into it — a product version is a fixed point, not a moving target. Once the Remediation Report is reviewed and accepted, its output becomes the Knowledge Base component of the **next** product version.
