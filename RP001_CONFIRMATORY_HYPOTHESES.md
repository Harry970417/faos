...
F_INST_01 shows positive cross-sectional Rank IC at t+1, t+3, and t+5 during the pre-break period (dates before the locked break interval, per `RP001_DEVIATION_POLICY.md`'s treatment of the break date).
...
No new feature. No new grouping variable. No new breakpoint search — the break interval is inherited from the original study as a locked input (see `RP001_FULL_UNIVERSE_SPEC.md` and `RP001_DEVIATION_POLICY.md` for how the break date is handled when full-universe data may not align perfectly with the original sample's calendar). No portfolio, performance, or trading-strategy hypothesis of any kind.
