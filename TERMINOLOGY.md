...
**Post-Exploration Prospective Confirmatory Protocol Lock** (equivalently: prospective confirmatory registration after exploratory analysis) — the formal, precise term for what RP-001's Protocol Lock actually is: the five confirmatory hypotheses (H-C1–H-C5) were derived from, and locked *after*, the already-completed exploratory-phase analysis (50-stock sample), then fixed *before* any full-universe confirmatory data was pulled or examined (Deviation D-10, `research/RP001_PHASE2A_DEVIATION_LOG.md`). Use this term (or "pre-registered *confirmatory* hypotheses"/"pre-registered relative to the confirmatory test") instead of a bare "pre-registered"/"pre-registration" whenever a reader unfamiliar with RP-001's two-phase design could otherwise infer that the *entire* study — including the exploratory phase — was preregistered before any data was seen. It was not, and does not need to have been: locking a confirmatory protocol after exploration is standard, sound practice, not a weaker form of pre-registration.
