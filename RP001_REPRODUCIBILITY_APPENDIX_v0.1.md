...
- `RP001_DATA_SNAPSHOT.json` — Milestone 0B, characterization sample manifest, 50 raw institutional files SHA-256 hashed in `rp001_data/raw_manifest.json`
...
`rp001_pull_characterization_sample.py`, `rp001_pull_price.py`, `rp001_pull_valuation.py`, `rp001_build_features.py` / `_v2.py`, `rp001_feature_tests.py`, `rp001_data_integrity_remediation.py`, `rp001_milestone_1c.py` / `_part2.py` / `_part3.py`, `rp001_1c_plus_setup.py` / `_analysis.py`, `rp001_breakpoint_analysis.py`, `rp001_regime_robustness.py`, `rp001_interaction_incremental.py`, `rp001_multiple_testing.py`.

## Regression Tests

`rp001_regression_test_20260619.py` — permanent guard against the 2026-06-19 non-trading-day contamination re-entering any future feature build.

## Full Decision Trail

`RP001_LOG.md` — every Research Decision from Milestone 0A through 1D, Decision/Reason/Evidence/Impact format, no entries removed or edited after the fact.
